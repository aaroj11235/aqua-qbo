...
def dec2symasym(data):

    '''
    Calculate symmetric and asymmetric parts of xr.DataArray and assign to a new coordinate called "compone    nt"
    '''
    # This function is modified so that it does not lose latitude data, like the ncl function
    lat_dim = data.dims.index('lat')
    data_sym = 0.5*(data.values + np.flip(data.values, axis = lat_dim))
    data_asym = 0.5*(data.values - np.flip(data.values, axis = lat_dim))
    data_sym = xr.DataArray(data_sym, dims = data.dims, coords = data.coords)
    data_asym = xr.DataArray(data_asym, dims = data.dims, coords = data.coords)
    data_out = xr.concat([data_sym, data_asym], "component")
    data_out = data_out.assign_coords({"component":["symmetric", "antisymmetric"]})

    return data_out

#def detrend(data):
#    xdetr = detrend(data.values, axis = timedim, type = 'linear')
#    data_detr = xr.DataArray(xdetr, dims = data.dims, coords = data.coords)
#    return data_detr
...
def spacetime_power(data, segsize=96, noverlap=60, spd=1, latitude_bounds=None, dosymmetries=False, rmvLowFrq=False):
    ...
    assert segsize-noverlap > 0, f"Error, inconsistent specification of segsize and noverlap results in stride of {rollstride}, but must be > 0."
    x_win = x_roll.construct("segments") #.dropna("time")  # WK99 say "2-month" overlap
    ##
...
def taperData(data):
    ...
    data_tp = taper*data_detr  # taper to zero
